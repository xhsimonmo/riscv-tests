...
// extern static char allocated_ram[TOTAL_DATA_SIZE];

// ==================
// Log initialisation
// ==================

void log_init(const char* allocated_ram)
// void log_init()
{
  const char* HEADSTARTS_BASE = allocated_ram;
  const char* HASH_TABLE_BASE = HEADSTARTS_BASE + NUM_HEADSTARTS*sizeof(headstart_t);
  // printf("LOG INIT: HEADSTARTS_BASE is 0x%x \n", HEADSTARTS_BASE);
  // printf("LOG INIT: HASH_TABLE_BASE is 0x%x \n", HASH_TABLE_BASE);
